...
class Solution {
 public:
  ...
  int ladderLength(string start, string end, unordered_set<string> &dict) {
    ...
    while (!sq.empty()) {
      std::string word = sq.front();
      int l = lq.front();
      sq.pop();
      lq.pop();

      if (CanTransform(word, end))
        return l + 1;
      ...
      for (int i = 0; i < word.size(); ++i) {
        ...
        for (int j = 'a'; j <= 'z'; ++j) {
          ...
          w[i] = j;
          std::unordered_set<std::string>::iterator it = dict.find(w);
          if (it != dict.end()) {
            sq.push(w);
            lq.push(l + 1);
            dict.erase(it);
...
int main(int argc, char *argv[]) {
  ...
  std::unordered_set<std::string> set2 = {"hot" ,"dot" ,"dog" ,"lot" ,"log"};

  std::cout << s.ladderLength("cet", "ism", set1) << std::endl;
  std::cout << s.ladderLength("hit", "cog", set2) << std::endl;
  return 0;
}
